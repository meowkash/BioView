...
class UsrpDeviceConfigPanel(QGroupBox):
    ...
    def __init__(self, config: UsrpConfiguration, parent=None):
    ...
    def modify_usrp_config(self, param_name, value, idx=None):
        ...
        try:
            ...
            self.logEvent.emit(
                "debug",
                f"{self.config.device_name}: Updated {param_name} to {value} successfully",
            )
        except Exception as e:
            self.logEvent.emit(
                "error", f"{self.config.device_name}: Updating {param_name} failed"
            )
